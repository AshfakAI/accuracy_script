...
def compare_json_files(original_file, test_file):
    ...
    original_keywords_by_lang = {entry['lang']: entry['keywords'] for entry in original_data['keywords_list']}
    test_keywords_by_lang = {entry['lang']: entry['keywords'] for entry in test_data['keywords_list']}
    ...
    if (original_data['keywords_list'] == []) and (test_data['keywords_list'] == []):
            ...
            if lang in test_keywords_by_lang:
                ...
                if original_keywords_set == test_keywords_set:
                    ...
                    keyword_mismatches.append({
                        "lang": lang,
                        "original_keywords": original_keywords_by_lang[lang],
                        "test_keywords": test_keywords_by_lang[lang]
                    })
            else:
                ...
                keyword_mismatches.append({
                    "lang": lang,
                    "original_keywords": original_keywords_by_lang[lang],
                    "test_keywords": 'Language missing in test file'
                })
    ...
    results_accuracy.append([os.path.basename(test_file), 'keyword', correct_keyword_count, incorrect_keyword_count])
    ...
    keyword_mismatch_str = "; ".join([f'"keywords": {mismatch["original_keywords"]} -> {mismatch["test_keywords"]}' 
                                      for mismatch in keyword_mismatches])
    results_mismatch.append([os.path.basename(test_file), 'keyword', keyword_mismatch_str])
...
def compare_folders(original_folder, test_folder):
    ...
    print(f"Overall accuracy for keyword: {overall_keyword_accuracy:.2f}%")

    # Save overall accuracy to the CSV file
    with open('output/keywordlist_accuracy.csv', 'w', newline='', encoding='utf-8') as accuracy_file:
        ...
        writer.writerow(['Overall accuracy', 'lang', f'{overall_lang_accuracy:.2f}%', ''])
        writer.writerow(['Overall accuracy', 'keyword', f'{overall_keyword_accuracy:.2f}%', ''])

    # Save mismatch results to CSV
    with open('output/keywordlist_mismatch.csv', 'w', newline='', encoding='utf-8') as mismatch_file:

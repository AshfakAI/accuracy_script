...
def compare_json_files(original_file, test_file):
    ...
    original_keywords_by_lang = {}
    for entry in original_data['abstracts']:
        for lang in entry['lang']:  # Iterate through all languages in the list
            original_keywords_by_lang[lang] = entry['text']

    test_keywords_by_lang = {}
    for entry in test_data['abstracts']:
        for lang in entry['lang']:  # Iterate through all languages in the list
            test_keywords_by_lang[lang] = entry['text']
    ...
    if original_data['abstracts'] == [] and test_data['abstracts']== []:
        ...
        try:
            if original_data["abstracts"][0]["lang"] == test_data["abstracts"][0]["lang"]:
                        correct_lang_count += 1

            else:
                    incorrect_lang_count += 1
                    lang_mismatches.append(original_data["abstracts"][0]["lang"])

            # Compare keywords for each lang
            if original_data["abstracts"][0]["lang"] == test_data["abstracts"][0]["lang"]:
                
                    similarity_score = ratio(original_data["abstracts"][0]["text"], test_data["abstracts"][0]["text"])
                    ...
                    if similarity_score>0.85:
                        ...
                        keyword_mismatches.append({
                            "lang": original_data["abstracts"][0]["lang"],
                            "original_text": original_data["abstracts"][0]["text"],
                            "test_text": test_data["abstracts"][0]["text"]
                        })
        except:
            incorrect_keyword_count += 1
            print(original_file)
...
def compare_folders(original_folder, test_folder):
    ...
    with open('output/abstract_subkey_accuracies.csv', 'w', newline='', encoding='utf-8') as accuracy_file:
        ...
        writer.writerow(['Overall accuracy', 'abstracts_lang', f'{overall_lang_accuracy:.2f}%', ''])
        writer.writerow(['Overall accuracy', 'abstracts_text', f'{overall_keyword_accuracy:.2f}%', ''])

    # Save mismatch results to CSV
    with open('output/abstract_subkey_mismatch.csv', 'w', newline='', encoding='utf-8') as mismatch_file:
